...
const envSchema = z.object({
  ...
  BULLMQ_CONCURRENCY: z.coerce.number().int().min(1).max(100).default(10),
  BULLMQ_STALLED_INTERVAL: z.coerce.number().int().min(1000).max(300000).default(30000),
  WHATSAPP_PROVIDER: z.enum(['mock', 'blip', 'meta']).default('mock'),
  BLIP_API_TOKEN: optionalText(),
  BLIP_FROM_NUMBER: optionalText(),
  BLIP_BASE_URL: z.string().url().default('https://http.msging.net'),
