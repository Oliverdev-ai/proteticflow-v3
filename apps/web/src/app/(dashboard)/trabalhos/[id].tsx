...
export default function JobDetailPage() {
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center h-[70vh] gap-6">
      ...
      <Muted className="font-black uppercase tracking-[0.3em] animate-pulse">Recuperando OS...</Muted>
    </div>
  );
                 ...
                 return (
                    <div key={s} className="flex flex-col items-center gap-3 flex-1 relative z-10">
                       <div className={cn(
                         ...
                         isPast ? 'bg-emerald-500 text-white shadow-emerald-500/10' : 
                         isCurrent ? 'bg-primary text-primary-foreground shadow-primary/30 scale-110 ring-4 ring-primary/10' : 
                         'bg-muted/40 text-muted-foreground border border-border opacity-30 grayscale'
                       )}>
                         {isPast ? <CheckCircle2 size={24} strokeWidth={3} /> : <Icon size={20} strokeWidth={3} />}
                       </div>
                       <div className="hidden lg:flex flex-col items-center">
                          <span className={cn(
                            "text-[9px] font-black uppercase tracking-widest text-center transition-all duration-700",
                            isCurrent ? 'text-primary' : isPast ? 'text-emerald-500' : 'text-muted-foreground opacity-30'
                          )}>
                             {JOB_STATUS_LABELS[s]}
                          </span>
                       </div>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-wrap gap-2 p-1.5 bg-card/50 backdrop-blur-sm border border-border rounded-[28px] shadow-sm w-fit">
          {TABS.map(t => (
            ...
            >
              <div className="flex items-center gap-3">
                 <t.icon size={16} strokeWidth={3} className={cn(tab === t.id ? "text-white" : "text-muted-foreground opacity-60 group-hover:text-primary transition-colors")} />
                     ...
                     { label: 'Número da Remessa', value: job.code, icon: Hash, color: 'text-primary' },
                      ...
                      <div key={label} className="group/item flex flex-col gap-3">
                         <div className="flex items-center gap-2">
                           ...
                           <Muted className="text-[10px] font-black uppercase tracking-[0.2em] group-hover/item:text-primary transition-colors">{label}</Muted>
                         </div>
                         <div className="flex items-center gap-4 pl-1">
                            <div className={cn("w-10 h-10 rounded-xl bg-muted/50 border border-border flex items-center justify-center opacity-60", color)}>
                               <Icon size={18} strokeWidth={2.5} />
                            </div>
                            <span className={cn("text-lg font-black tracking-tight leading-none", color)}>{value}</span>
                         </div>
                      </div>
                   ))}
                </div>
               ...
               <div className="overflow-x-auto">
                 <table className="w-full border-collapse">
                   ...
                   <tbody className="divide-y divide-border/50">
                     {job.items.map((item) => (
                       <tr key={item.id} className="group hover:bg-primary/[0.01] transition-all duration-300">
                         <td className="px-10 py-8">
                            <div className="flex items-center gap-4">
                               ...
                               <span className="text-sm font-black text-foreground uppercase tracking-tight group-hover:text-primary transition-colors">{item.serviceNameSnapshot}</span>
                            </div>
                         </td>
                         ...
                         <td className="px-10 py-8 text-right">
                           <span className={cn(
                             ...
                             Number(item.adjustmentPercent) > 0 ? "text-emerald-500 bg-emerald-500/10 border-emerald-500/20" : Number(item.adjustmentPercent) < 0 ? "text-destructive bg-destructive/10 border-destructive/20" : "text-muted-foreground bg-muted border-border"
                           )}>
                             {Number(item.adjustmentPercent) > 0 ? '+' : ''}{item.adjustmentPercent}%
                           </span>
                         </td>
                         <td className="px-10 py-8 text-right">
                            <div className="flex flex-col items-end gap-0.5">
                               <span className="text-lg font-black text-primary tabular-nums tracking-tighter group-hover:scale-105 transition-transform origin-right">{formatBRL(item.totalCents)}</span>
                               <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-[0.2em] opacity-40">Subtotal Compilado</span>
                            </div>
                         </td>
                       </tr>
                     ))}
                   ...
                   <tfoot className="bg-primary/[0.03] border-t-2 border-primary/20">
                     <tr>
                       ...
                       <td className="px-10 py-10 text-right">
                          <div className="flex flex-col items-end gap-1">
                             <span className="text-4xl font-black text-primary tracking-tighter tabular-nums leading-none mb-1">{formatBRL(job.totalCents)}</span>
            ...
            <ScaleIn className="premium-card p-12">
               <div className="flex items-center gap-4 mb-14 border-b border-border/50 pb-6">
                  ...
                  <div className="flex flex-col gap-0.5">
                     ...
                     <Muted className="text-[10px] font-black uppercase tracking-[0.2em]">Fluxo de controle e auditoria de status</Muted>
                  </div>
               </div>
                    ...
                    <div key={log.id} className="flex gap-10 relative animate-in slide-in-from-left-4 duration-500" style={{ animationDelay: `${idx * 100}ms` }}>
                      <div className="relative z-10">
                        <div className={cn(
                          ...
                          log.toStatus === job.status ? "bg-primary text-white scale-110" : "bg-muted-foreground/20 text-white opacity-40 hover:opacity-100"
                        )}>
                          {log.toStatus === job.status ? <div className="w-2.5 h-2.5 bg-white rounded-full animate-ping" /> : <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                        </div>
                      </div>
                      <div className="flex-1 -mt-1 bg-muted/20 p-8 rounded-[32px] border border-border/50 hover:border-primary/30 transition-all duration-500 group relative overflow-hidden">
                        {log.toStatus === job.status && <div className="absolute top-0 right-0 w-24 h-24 bg-primary/[0.03] rounded-full blur-2xl -mr-12 -mt-12" />}
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6 relative">
                          <div className="flex flex-col gap-1">
                             <div className="flex items-center gap-3">
                                <span className={cn(
                                   ...
                                   log.toStatus === 'cancelled' ? "bg-destructive/10 text-destructive border-destructive/20" : "bg-primary/10 text-primary border-primary/20"
                                )}>
                                   {JOB_STATUS_LABELS[log.toStatus as JobStatus] || log.toStatus}
                                </span>
                                {log.fromStatus && <ChevronRight size={12} className="text-muted-foreground/30" />}
                                {log.fromStatus && (
                                   <span className="text-[9px] font-bold text-muted-foreground opacity-30 uppercase tracking-widest">{JOB_STATUS_LABELS[log.fromStatus as JobStatus] || log.fromStatus}</span>
                                )}
                             </div>
                          ...
                          <div className="flex flex-col lg:items-end gap-0.5">
                             <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{new Date(log.createdAt).toLocaleDateString('pt-BR')}</span>
                             <Muted className="text-[10px] font-bold uppercase tracking-widest opacity-40">{new Date(log.createdAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</Muted>
                          </div>
                        </div>
                        ...
                        {log.notes && (
                          <div className="relative p-5 bg-card/60 border border-border/40 rounded-2xl italic text-[11px] text-muted-foreground leading-relaxed shadow-inner">
                            <span className="absolute -top-3 left-6 px-2 bg-card text-[8px] font-black text-primary uppercase tracking-widest border border-border/40 rounded-full">Nota de Auditoria</span>
                            "{log.notes}"
                          </div>
                        )}
                      </div>
          ...
          <ScaleIn className="w-full max-w-lg">
            <div className="premium-card p-12 flex flex-col gap-10 relative shadow-2xl border-destructive/20 overflow-hidden bg-white dark:bg-[#0a0a0b]">
               ...
               <div className="flex gap-4 pt-6 border-t border-border/50 relative">
                  ...
                  >
                    {changeStatus.isPending ? <Loader2 size={16} className="animate-spin" /> : <XCircle size={18} strokeWidth={3} />}
