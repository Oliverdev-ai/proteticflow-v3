...
export default function JobCreatePage() {
  ...
  const inputClass = "w-full bg-muted border border-border rounded-xl px-4 py-3 text-sm font-semibold placeholder:text-muted-foreground/30 focus:outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary/50 transition-all shadow-inner";
  const labelClass = "block text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-2 ml-1";
            ...
            {STEPS.map((s, i) => (
               <div key={i} className="flex flex-col items-center gap-3 flex-1 relative z-10">
                  <div className={cn(
                    ...
                    i < step ? 'bg-emerald-500 text-white shadow-emerald-500/20' : 
                    i === step ? 'bg-primary text-primary-foreground shadow-primary/30 scale-110 ring-4 ring-primary/10' : 
                    'bg-muted text-muted-foreground border border-border opacity-40'
                  )}>
                    {i < step ? <CheckCircle2 size={24} strokeWidth={2.5} /> : <s.icon size={20} strokeWidth={2.5} />}
                  </div>
               ...
               <div className="flex items-center justify-between ml-1 border-b border-border/50 pb-4">
                  ...
                  {!clientId && osNumber && (
                    <div className="flex items-center gap-2 px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full animate-pulse">
                       ...
                       <span className="text-[9px] font-black uppercase tracking-widest text-amber-500">Buscando Registro...</span>
                    </div>
                  )}
               </div>
                    ...
                    <div className="md:col-span-2 p-6 rounded-3xl bg-emerald-500/[0.03] border-2 border-emerald-500 ring-4 ring-emerald-500/10 flex items-center justify-between shadow-2xl animate-in zoom-in-95 duration-500">
                       <div className="flex items-center gap-6">
                          ...
                          <div className="flex flex-col gap-1">
                             <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Parceiro Identificado na OS #{osNumber}</span>
                             <Large className="text-2xl font-black tracking-tighter leading-none">{resolveQuery.data.clientName}</Large>
                          </div>
                       </div>
                       <button className="text-[10px] font-black uppercase tracking-widest text-emerald-500 hover:brightness-110 transition-all">Trocar Parceiro</button>
                    </div>
                  )}

                  {!resolveQuery.data && clientsData?.data.map(c => (
                    <button 
                      key={c.id} 
                    ...
                    >
                      <div className={cn(
                        ...
                        clientId === c.id ? "bg-primary text-white border-primary shadow-primary/20" : "bg-card border-border text-muted-foreground opacity-40 group-hover:opacity-100"
                      )}>
                         {clientId === c.id ? <Check size={20} strokeWidth={3} /> : <User size={20} />}
                      </div>
                      <div className="flex flex-col gap-0.5 flex-1">
                         <span className={cn("text-sm font-black tracking-tight leading-none mb-1", clientId === c.id ? 'text-primary' : 'text-foreground')}>{c.name}</span>
                         <div className="flex items-center gap-2 opacity-40">
                            <Landmark size={10} />
                            <span className="text-[10px] font-bold uppercase tracking-widest leading-none">{c.document || 'Sem Documento'}</span>
                         </div>
                      </div>
                      {clientId === c.id && <div className="absolute top-0 right-0 w-24 h-24 bg-primary/5 rounded-full -mr-12 -mt-12 group-hover:scale-125 transition-transform duration-700" />}
                    </button>
                  ))}
               </div>
            </div>
          ...
          <ScaleIn className="p-8 flex flex-col gap-10">
             <div className="flex items-center justify-between pb-8 border-b border-border/50 relative">
                ...
                <div className="flex flex-col items-end gap-1">
                   <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Faturamento Estimado</span>
                   ...
                   {selectedTableId && (
                     <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                        ...
                        {priceItemsData?.data.map(pi => (
                          ...
                          >
                            <div className="flex flex-col gap-0.5">
                               <span className="text-[11px] font-black text-foreground group-hover:text-primary transition-colors tracking-tight leading-tight">{pi.name}</span>
                               <span className="text-[9px] font-bold text-muted-foreground opacity-50">{formatBRL(pi.priceCents)}</span>
                            </div>
                ...
                <div className="md:col-span-8 space-y-4">
                   ...
                   <div className="flex flex-col gap-3 min-h-[300px]">
                      {items.length > 0 ? items.map((item, i) => (
                        <div key={i} className="flex gap-4 items-center group p-4 rounded-[24px] bg-card border border-border hover:border-primary/30 transition-all duration-500 animate-in slide-in-from-right-4">
                           ...
                           <div className="flex-1 flex flex-col gap-1">
                              ...
                              <div className="flex items-center gap-4">
                                 <div className="flex items-center gap-2">
                                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Qtd:</span>
                                    <input 
                                      type="number" 
                                      value={item.quantity} 
                                 ...
                                 <div className="flex items-center gap-2">
                                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Un:</span>
                                    <div className="relative">
                                       <span className="absolute left-0 top-1/2 -translate-y-1/2 text-[9px] font-black text-emerald-500/40">R$</span>
                                       <input 
                                        type="number" 
                                        value={item.unitPriceCents / 100} 
                                        onChange={e => updateItem(i, 'unitPriceCents', Number(e.target.value) * 100)} 
                           ...
                           <div className="flex flex-col items-end gap-1 px-4 border-l border-border/50">
                              <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest opacity-40">Subtotal</span>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
                ...
                <div className="group/field relative">
                  <label className={labelClass}>Nome do Paciente *</label>
                   <div className="relative">
                      <User className="absolute left-4 top-1/2 -translate-y-1/2 text-primary/30 group-focus-within/field:text-primary transition-colors duration-300" size={18} strokeWidth={2.5} />
                      <input 
                ...
                <div className="group/field relative">
                  <label className={labelClass}>Escala de Cor</label>
                   <div className="relative">
                      <Activity className="absolute left-4 top-1/2 -translate-y-1/2 text-primary/30 group-focus-within/field:text-primary transition-colors duration-300" size={18} strokeWidth={2.5} />
                      <input 
                ...
                <div className="md:col-span-2 group/field relative">
                  ...
                  <div className="relative max-w-md">
                    <Calendar className="absolute left-5 top-1/2 -translate-y-1/2 text-primary/40 group-focus-within/field:text-primary transition-colors duration-300" size={20} strokeWidth={3} />
                    <input 
                      type="date" 
                      value={details.deadline} 
                      onChange={e => setDetails(d => ({ ...d, deadline: e.target.value }))} 
                      className={cn(inputClass, "pl-16 bg-primary/5 border-primary/20 text-primary font-black text-lg focus:ring-primary/10 tracking-tight cursor-pointer")} 
                    />
                  </div>
                </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-background/80 backdrop-blur-xl border-t border-border/50 z-[100] md:static md:p-0 md:bg-transparent md:border-0 md:mt-2">
        <div className="flex gap-4 max-w-5xl mx-auto w-full">
          {step > 0 && (
            ...
            >
              <ArrowLeft size={16} strokeWidth={3} className="group-hover:-translate-x-1 transition-transform" /> Retroceder
            </button>
          )}
          {step < 3 ? (
            <button 
            ...
            >
              {createMutation.isPending ? <Loader2 className="animate-spin" size={16} strokeWidth={3} /> : <Check size={18} strokeWidth={3} />}
