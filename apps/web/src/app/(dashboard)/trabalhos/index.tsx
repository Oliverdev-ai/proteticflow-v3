  ...
  return (
    <div className="flex flex-col items-end gap-1">
       <div className={cn(
        ...
        {isOverdue ? <AlertCircle size={14} strokeWidth={3} /> : <Calendar size={14} className="opacity-40" />}
    ...
    <PageTransition className="flex flex-col gap-8 h-full overflow-auto p-4 md:p-1 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-wrap items-center gap-4 bg-card/30 backdrop-blur-sm p-4 rounded-[32px] border border-border shadow-sm">
        <div className="relative flex-1 min-w-[320px] group">
          <Search size={18} className="absolute left-5 top-1/2 -translate-y-1/2 text-muted-foreground group-focus-within:text-primary transition-colors duration-300" />
          <input 
            value={search} 
        ...
        <div className="flex items-center gap-2">
           <div className="relative">
              <Filter className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground/40 pointer-events-none" size={16} strokeWidth={3} />
              <select 
                value={status} 
                onChange={e => { setStatus(e.target.value as JobStatus | ''); setCursor(undefined); }} 
           ...
           <button 
            onClick={() => { setOverdue(v => !v); setCursor(undefined); }} 
            className={cn(
        ...
        {isLoading ? (
          <div className="py-32 flex flex-col items-center justify-center gap-6">
            <div className="relative">
              ...
              <Activity className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-primary anim-pulse" size={24} />
            </div>
            <Muted className="font-black uppercase tracking-[0.3em] animate-pulse">Sincronizando Ordens de Serviço...</Muted>
          </div>
        ) : error ? (
          <div className="premium-card p-16 flex flex-col items-center gap-6 border-destructive/20 bg-destructive/[0.02]">
            ...
            <div className="text-center space-y-2">
               <Large className="text-destructive font-black tracking-tight">Erro ao recuperar dados</Large>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5"># Identificador</th>
                    <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5 hidden md:table-cell">Mandatário (Cliente)</th>
                    <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5 hidden sm:table-cell">Paciente Final</th>
                    <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Status do Workflow</th>
                    <th className="text-right text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5 hidden md:table-cell">Valoração</th>
                    <th className="text-right text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">DeadLine</th>
                    <th className="px-8 py-5 w-12" />
                  </tr>
                </thead>
                <tbody className="divide-y divide-border/50">
                  {data?.data.map((job) => (
                    <tr key={job.id} className="group hover:bg-primary/[0.01] transition-all duration-300">
                      <td className="px-8 py-6">
                         <div className="flex items-center gap-4">
                            ...
                            <span className="text-sm font-black text-foreground tracking-tighter group-hover:text-primary transition-colors">{job.code}</span>
                         </div>
                      </td>
                      <td className="px-8 py-6 hidden md:table-cell">
                        <div className="flex items-center gap-3">
                           <Building2 size={16} className="text-muted-foreground/30" />
                           <span className="text-xs font-black text-foreground tracking-tight uppercase">{job.clientName}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6 hidden sm:table-cell">
                         <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-bold text-muted-foreground tracking-tight">{job.patientName ?? 'Não informado'}</span>
                         </div>
                      </td>
                      <td className="px-8 py-6">
                      ...
                      <td className="px-8 py-6 text-right hidden md:table-cell">
                         <div className="flex flex-col items-end gap-0.5">
                            <span className="text-sm font-black text-foreground tabular-nums tracking-tighter">{formatBRL(job.totalCents)}</span>
                            <div className="flex items-center gap-1 opacity-40">
                               <DollarSign size={10} className="text-muted-foreground" />
                               <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground">Líquido</span>
                            </div>
                         </div>
                      </td>
                      <td className="px-8 py-6 text-right">
                        <DeadlineCell deadline={job.deadline.toString()} status={job.status as JobStatus} />
                      </td>
                      <td className="px-8 py-6 text-right">
                        <Link 
                          to={`/trabalhos/${job.id}`} 
