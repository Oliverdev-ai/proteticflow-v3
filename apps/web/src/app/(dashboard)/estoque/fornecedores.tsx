...
import { 
  Users, Search, Plus, ChevronLeft,
  Mail, Phone, FileText, User, Building2, X, Loader2, 
...
export default function SuppliersPage() {
  ...
  const { data, isLoading } = trpc.inventory.listSuppliers.useQuery({ search: search || undefined, page: 1, limit: 50 });
      ...
      <ScaleIn className="space-y-4">
        {isLoading ? (
          <div className="p-20 flex flex-col items-center justify-center gap-4">
             ...
             <Muted className="animate-pulse font-black uppercase tracking-[0.2em]">Buscando parceiros comerciais...</Muted>
          </div>
        ) : suppliers.length === 0 ? (
          <div className="p-12">
            <EmptyState 
          ...
          suppliers.map((s) => (
            ...
            >
              <div className="flex items-center gap-5">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex flex-wrap gap-4 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                    {s.cnpj && <span className="flex items-center gap-1.5"><FileText size={12} className="opacity-40" /> {s.cnpj}</span>}
                    {s.email && <span className="flex items-center gap-1.5"><Mail size={12} className="opacity-40" /> {s.email}</span>}
                    {s.phone && <span className="flex items-center gap-1.5"><Phone size={12} className="opacity-40" /> {s.phone}</span>}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6">
                 {s.isActive ? (
                   <span className="text-[9px] font-black uppercase tracking-[0.2em] px-3 py-1 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-full">Ativo</span>
                 ) : (
                   <span className="text-[9px] font-black uppercase tracking-[0.2em] px-3 py-1 bg-muted text-muted-foreground border border-border rounded-full">Inativo</span>
                 )}
                 ...
                 <button 
                   ...
                   className={cn(
                     ...
                     s.isActive ? "bg-primary/5 text-primary border-primary/20 hover:bg-primary hover:text-primary-foreground" : "bg-muted text-muted-foreground border-border hover:border-primary/40"
                   )}
                   title={s.isActive ? "Desativar parceiro" : "Ativar parceiro"}
                 >
                   <ChevronRight size={20} strokeWidth={3} className={cn("transition-transform", s.isActive && "rotate-90")} />
                 </button>
              </div>
            </div>
          ))
          ...
          <ScaleIn className="w-full max-w-xl">
            <div className="premium-card p-10 flex flex-col gap-10 relative shadow-2xl border-primary/20 overflow-hidden">
               ...
               <div className="flex justify-between items-start relative">
                 <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      <Muted className="text-[10px] font-black uppercase tracking-[0.2em]">Cadastro de parceiro comercial</Muted>
                    </div>
                 </div>
                 <button 
                 ...
                 <div>
                    ...
                    <div className="relative">
                       <input 
                         ...
                         className={inputClass} 
                       />
                       <User className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground/30 opacity-50" size={18} />
                    </div>
                 </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input 
                          ...
                          className={inputClass} 
                        />
                        <Mail className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground/30 opacity-50" size={18} />
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        <input 
                          ...
                          className={inputClass} 
                        />
                        <Phone className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground/30 opacity-50" size={18} />
                      </div>
                    </div>
                 </div>
               </div>
               ...
               <div className="flex gap-4 pt-6 mt-4 border-t border-border/50 relative">
                  ...
                  >
                    {createSupplier.isPending ? <Loader2 size={16} className="animate-spin" /> : <><CheckCircle2 size={16} strokeWidth={3} /> Salvar Fornecedor</>}
                  </button>
               </div>
            </div>
          </ScaleIn>
