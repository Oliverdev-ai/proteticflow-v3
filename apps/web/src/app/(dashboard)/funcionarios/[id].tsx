           ...
           <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
             <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
               <div className="flex items-center justify-between">
                 ...
                 {isEditing && (
                    <button type="submit" className="text-primary hover:text-primary flex items-center gap-1 text-xs font-bold uppercase transition-colors">
               ...
               <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                 ...
                 <div>
                   ...
                   {isEditing ? (
                     ...
                     <p className="text-sm text-zinc-200">R$ {((employee.baseSalaryCents || 0) / 100).toLocaleString('pt-BR')}</p>
                   )}
                 </div>
                 <div>
                 ...
                 <tbody>
                   {timesheets?.map((entry) => (
                     <tr key={entry.id} className="border-b border-zinc-800 last:border-b-0">
                       <td className="px-4 py-3 text-zinc-200">{new Date(entry.date).toLocaleDateString('pt-BR')}</td>
                       ...
                       <td className="px-4 py-3 text-zinc-300">{entry.hoursWorked ? `${Number(entry.hoursWorked).toFixed(2)}h` : '-'}</td>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-zinc-800">
                      ...
                      <th className="pb-3 text-[10px] font-bold text-zinc-500 uppercase text-right">Data</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-800">
                    {assignments?.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="py-8 text-center text-xs text-zinc-500">Nenhuma OS atribuída ainda.</td>
                      </tr>
                    ) : (
                      assignments?.map((as) => (
                        <tr key={as.id} className="text-sm">
                          ...
                          <td className="py-3">
                            {as.commissionAmountCents ? `R$ ${(as.commissionAmountCents / 100).toFixed(2)}` : <span className="text-zinc-600 text-[10px]">Aguardando cálculo</span>}
                          </td>
        ...
        <div className="space-y-6">
           <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
             <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowSkillForm(true)} className="p-1.5 bg-zinc-800 hover:bg-zinc-700 rounded-lg text-primary transition-colors">
                   <Plus size={14} />
                </button>
             </div>
             ...
             {showSkillForm && (
                <div className="p-3 bg-zinc-950 border border-zinc-800 rounded-xl space-y-3">
                   ...
                   <div className="flex gap-2">
                      <button onClick={() => setShowSkillForm(false)} className="flex-1 py-1 text-[10px] text-zinc-500 border border-zinc-800 rounded-md">Cancelar</button>
                      <button 
                        onClick={() => {
             ...
             <div className="space-y-3">
                {skills?.length === 0 && !showSkillForm && <p className="text-xs text-zinc-600 italic">Nenhuma habilidade cadastrada.</p>}
                {skills?.map(skill => (
                  <div key={skill.id} className="flex items-center justify-between group">
                    <div>
                      ...
                      <div className="flex gap-1 mt-1">
                        {[1,2,3,4].map(l => (
                          <div key={l} className={`h-1 w-4 rounded-full ${l <= skill.level ? 'bg-primary' : 'bg-zinc-800'}`} />
                        ))}
                      </div>
                    </div>
                    <button 
