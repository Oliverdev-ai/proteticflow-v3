...
export default function LivroCaixaPage() {
  ...
  const inputClass = "w-full bg-muted border border-border rounded-xl px-4 py-2.5 text-sm font-semibold placeholder:text-muted-foreground/30 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/50 transition-all shadow-inner";
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <ScaleIn className="premium-card p-6 flex flex-col gap-6 relative overflow-hidden group">
           ...
           <div className="flex items-center justify-between relative">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500/60">Total Créditos</span>
           </div>
           <div className="flex flex-col gap-0.5 relative">
              ...
              <Muted className="text-[9px] font-bold uppercase tracking-widest">Entradas acumuladas</Muted>
           </div>
        </ScaleIn>

        <ScaleIn delay={0.1} className="premium-card p-6 flex flex-col gap-6 relative overflow-hidden group">
           ...
           <div className="flex items-center justify-between relative">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-destructive/60">Total Débitos</span>
           </div>
           <div className="flex flex-col gap-0.5 relative">
              <Large className="text-3xl font-black tracking-tighter text-destructive tabular-nums">
                {balance ? formatBRL(balance.totalDebits) : '—'}
              </Large>
              <Muted className="text-[9px] font-bold uppercase tracking-widest">Saídas acumuladas</Muted>
           </div>
        </ScaleIn>

        <ScaleIn delay={0.2} className={cn(
          "premium-card p-6 flex flex-col gap-6 relative overflow-hidden",
          (balance?.netBalance ?? 0) >= 0 ? "bg-emerald-500/[0.03] border-emerald-500/20 shadow-emerald-500/5 shadow-2xl" : "bg-destructive/[0.03] border-destructive/20 shadow-destructive/5 shadow-2xl"
        )}>
           <div className="flex items-center justify-between relative">
              <div className={cn(
                "w-12 h-12 flex items-center justify-center rounded-[20px] shadow-lg transition-transform duration-500 hover:rotate-12",
                (balance?.netBalance ?? 0) >= 0 ? "bg-emerald-500 text-white shadow-emerald-500/20" : "bg-destructive text-white shadow-destructive/20"
              )}>
                 <Activity size={24} strokeWidth={2.5} />
              </div>
              <div className={cn(
                "flex items-center gap-1.5 px-3 py-1 rounded-full border text-[9px] font-black uppercase tracking-widest",
                (balance?.netBalance ?? 0) >= 0 ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-destructive/10 text-destructive border-destructive/20"
              )}>
                 { (balance?.netBalance ?? 0) >= 0 ? <ArrowUpRight size={10} strokeWidth={3} /> : <ArrowDownRight size={10} strokeWidth={3} />}
                 Saldo Real
              </div>
           </div>
           <div className="flex flex-col gap-0.5 relative">
              ...
              <Muted className="text-[9px] font-black uppercase tracking-widest opacity-60">Disponibilidade líquida</Muted>
           </div>
        </ScaleIn>
      </div>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Timestamp / Data</th>
                  <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Descrição da Transação</th>
                  <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Subcategoria</th>
                  <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Modalidade</th>
                  <th className="text-right text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Montante Final</th>
                </tr>
              </thead>
                  ...
                  <tr>
                    <td colSpan={5} className="py-24 text-center">
                       <div className="flex flex-col items-center gap-4">
                          ...
                          <Muted className="animate-pulse font-black uppercase tracking-[0.2em]">Varrendo registros contábeis...</Muted>
                       </div>
                    </td>
                  </tr>
                ) : entries.length === 0 ? (
                  ...
                  <tr key={entry.id} className="group hover:bg-primary/[0.01] transition-colors">
                    <td className="px-8 py-6">
                       <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col gap-0.5">
                             <span className="text-xs font-black text-foreground uppercase tracking-tight">{new Date(entry.referenceDate).toLocaleDateString('pt-BR')}</span>
                             <Muted className="text-[9px] font-bold uppercase tracking-widest opacity-40">Efetivação</Muted>
                          </div>
                       </div>
                    </td>
                    <td className="px-8 py-6">
                       <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col gap-0.5">
                             <span className="text-sm font-black text-foreground tracking-tight leading-none mb-1">{entry.description}</span>
                             <div className="flex items-center gap-1.5 opacity-60">
                                <Landmark size={10} className="text-muted-foreground" />
                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest italic leading-none whitespace-nowrap">ID_{entry.id.toString().padStart(6, '0')}</span>
                             </div>
                          </div>
                       </div>
                    </td>
                    <td className="px-8 py-6">
                       <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-muted/50 border border-border w-fit group-hover:bg-muted group-hover:border-primary/20 transition-colors">
                          <Tag size={12} className="text-primary/40 group-hover:text-primary transition-colors" />
                          <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground group-hover:text-foreground transition-colors">{entry.category ?? 'Operacional'}</span>
                       </div>
                    </td>
                    <td className="px-8 py-6">
                       <div className={cn(
                        ...
                        entry.type === 'credit' 
                         ...
                         {entry.type === 'credit' ? <ArrowUpRight size={10} strokeWidth={3} /> : <ArrowDownRight size={10} strokeWidth={3} />}
                    ...
                    <td className="px-8 py-6 text-right tabular-nums">
                       <div className={cn(
                        ...
                        entry.type === 'credit' ? 'text-emerald-500 group-hover:scale-105 origin-right' : 'text-destructive group-hover:scale-105 origin-right'
                      )}>
                        {entry.type === 'debit' ? '− ' : '+ '}
          ...
          <ScaleIn className="w-full max-w-xl">
            <div className="premium-card p-10 flex flex-col gap-10 relative shadow-2xl border-primary/20 overflow-hidden">
               ...
               <div className="flex justify-between items-start relative">
                 <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      <Muted className="text-[10px] font-black uppercase tracking-[0.2em]">Movimentação manual para o livro caixa</Muted>
                    </div>
                 </div>
                 <button 
                      ...
                      {(['credit', 'debit'] as const).map(t => (
                        ...
                        >
                          <div className={cn(
                            ...
                            form.type === t ? "bg-white/20 text-white" : "bg-muted text-muted-foreground"
                          )}>
                             {t === 'credit' ? <TrendingUp size={20} strokeWidth={3} /> : <TrendingDown size={20} strokeWidth={3} />}
                          </div>
                          <span className="text-[10px] font-black uppercase tracking-[0.2em]">{t === 'credit' ? 'Crédito / Entrada' : 'Débito / Saída'}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div className="md:col-span-2">
                        ...
                        <div className="relative">
                           ...
                           />
                           <FileText className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground/30 opacity-50" size={18} />
                        </div>
                     </div>
                     ...
                     <div>
                        ...
                        <div className="relative">
                           <input 
                              ...
                              placeholder="0.00"
                              className={cn(inputClass, "font-mono font-bold text-lg", form.type === 'debit' ? "text-destructive" : "text-emerald-500")} 
                           />
                           <DollarSign className={cn("absolute right-4 top-1/2 -translate-y-1/2 opacity-30", form.type === 'debit' ? "text-destructive" : "text-emerald-500")} size={18} />
                        </div>
                     </div>
               ...
               <div className="flex gap-4 pt-6 mt-4 border-t border-border/50 relative">
                  ...
                  >
                    {createEntry.isPending ? <Loader2 size={16} className="animate-spin" /> : <><CheckCircle2 size={16} strokeWidth={3} /> Efetivar Lançamento</>}
                  </button>
               </div>
            </div>
          </ScaleIn>
