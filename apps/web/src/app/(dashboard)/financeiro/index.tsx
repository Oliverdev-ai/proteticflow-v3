...
import {
  ...
  FileText, BookOpen, BarChart3, ChevronRight,
  ArrowUpRight, Receipt, CreditCard, PieChart,
  ArrowDownRight, Landmark, ArrowRight
} from 'lucide-react';
...
function SummaryCard({
  label, value, icon: Icon, sub, color, trend,
}: { 
  label: string; 
  value: string; 
  ...
  return (
    <ScaleIn className="premium-card p-6 flex flex-col gap-4 group relative overflow-hidden">
      ...
      <div className="flex items-start justify-between relative">
        <div className={cn("p-3 rounded-2xl border shadow-inner transition-all duration-500 group-hover:scale-110", colorMap[color])}>
          <Icon size={22} strokeWidth={2.5} />
        </div>
        {trend && (
          <div className={cn(
            "flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest border",
            trend.positive ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-destructive/10 text-destructive border-destructive/20"
          )}>
            {trend.positive ? <ArrowUpRight size={10} strokeWidth={3} /> : <ArrowDownRight size={10} strokeWidth={3} />}
            {trend.value}
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col gap-0.5 relative">
        ...
        {sub && <Muted className="text-[10px] font-bold uppercase tracking-widest opacity-60">{sub}</Muted>}
      </div>
    </ScaleIn>
  );
}
...
const MODULES = [
  { id: 'ar', label: 'Contas a Receber', href: '/financeiro/contas-receber', icon: TrendingUp, desc: 'Gestão de faturamento' },
  { id: 'ap', label: 'Contas a Pagar', href: '/financeiro/contas-pagar', icon: TrendingDown, desc: 'Controle de despesas' },
  { id: 'cashflow', label: 'Fluxo de Caixa', href: '/financeiro/fluxo-caixa', icon: BarChart3, desc: 'Visão de caixa real' },
  { id: 'cashbook', label: 'Livro Caixa', href: '/financeiro/livro-caixa', icon: BookOpen, desc: 'Lançamentos detalhados' },
  { id: 'closing', label: 'Fechamentos', href: '/financeiro/fechamento', icon: FileText, desc: 'Apuração de resultados' },
] as const;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="flex items-center gap-4 ml-1">
             ...
             <Muted className="text-[10px] font-black uppercase tracking-[0.3em]">Módulos Operacionais</Muted>
          </div>
              ...
              >
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 flex items-center justify-center rounded-[24px] bg-secondary border border-border group-hover:bg-primary/10 group-hover:text-primary transition-all duration-500 shadow-inner group-hover:rotate-6">
                    <Icon size={28} className="text-muted-foreground group-hover:text-primary transition-colors" />
                  </div>
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm font-black text-foreground group-hover:text-primary transition-colors tracking-tight">{label}</span>
                    <Muted className="text-[10px] font-bold uppercase tracking-widest opacity-60">{desc}</Muted>
                  </div>
                </div>
        ...
        <div className="lg:col-span-4 space-y-6">
           <div className="flex items-center gap-4 ml-1">
             <div className="w-1 h-3 bg-primary rounded-full" />
             <Muted className="text-[10px] font-black uppercase tracking-[0.3em]">Insights Flash</Muted>
           </div>

           <ScaleIn delay={0.2} className="premium-card p-10 bg-primary/5 border-primary/20 relative overflow-hidden">
              ...
              <div className="flex flex-col gap-6 relative">
                 ...
                 <div className="flex flex-col gap-2">
                    <p className="text-xl font-black tracking-tighter leading-tight">Saúde Financeira estável</p>
                    <p className="text-xs text-muted-foreground font-medium leading-relaxed">
                       O laboratório apresenta um índice de inadimplência de <span className="text-emerald-500 font-bold">2.4%</span> este mês. Continue monitorando os vencimentos em atraso.
                    </p>
                 </div>
           ...
           <ScaleIn delay={0.4} className="premium-card p-6 flex items-center gap-4 group cursor-pointer hover:bg-muted/50 transition-colors border-dashed">
              ...
              <div className="flex flex-col gap-0.5">
                 <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Dica de Gestão</span>
                 <span className="text-[11px] font-bold text-foreground">Otimize suas compras de estoque centralizadas.</span>
              </div>
           </ScaleIn>
        </div>
      </div>
