...
function StatusBadge({ status }: { status: string }) {
  const { label, cls, icon: Icon } = STATUS_MAP[status] ?? { label: status, cls: 'bg-muted text-muted-foreground', icon: FileText };
  return (
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Período Fiscal</th>
                  <th className="text-right text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Volatilidade (OS)</th>
                  <th className="text-right text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Faturamento Bruto</th>
                  <th className="text-right text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Liquidez (Pago)</th>
                  <th className="text-right text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Inadimplência (AR)</th>
                  <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-8 py-5">Status</th>
                </tr>
              </thead>
                  ...
                  <tr>
                    <td colSpan={6} className="py-24 text-center">
                       <div className="flex flex-col items-center gap-4">
                          ...
                          <Muted className="animate-pulse font-black uppercase tracking-[0.2em]">Consolidando faturas mensais...</Muted>
                       </div>
                    </td>
                  </tr>
                ) : rows.length === 0 ? (
                  ...
                  <tr key={c.id} className="group hover:bg-primary/[0.01] transition-colors">
                    <td className="px-8 py-6">
                       <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col gap-0.5">
                             <span className="text-sm font-black text-foreground uppercase tracking-tight">{c.period}</span>
                             <Muted className="text-[10px] font-bold uppercase tracking-widest opacity-60">Competência</Muted>
                          </div>
                       </div>
                    </td>
                    <td className="px-8 py-6 text-right">
                       <div className="flex flex-col items-end gap-1">
                          <span className="text-sm font-black text-foreground tabular-nums">{c.totalJobs}</span>
                          <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Trabalhos OS</span>
                       </div>
                    </td>
                    <td className="px-8 py-6 text-right">
          ...
          <ScaleIn className="w-full max-w-lg">
            <div className="premium-card p-10 flex flex-col gap-10 relative shadow-2xl border-primary/20 overflow-hidden">
               ...
               <div className="flex justify-between items-start relative">
                 <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      <Muted className="text-[10px] font-black uppercase tracking-[0.2em]">Processamento de faturamento consolidado</Muted>
                    </div>
                 </div>
                 <button 
               ...
               <div className="flex flex-col gap-6 relative">
                  <div className="p-5 bg-muted/50 rounded-2xl border border-border text-xs text-muted-foreground leading-relaxed">
                    <div className="flex items-center gap-2 mb-2 text-primary">
                       ...
                       <span className="font-black uppercase tracking-widest text-[9px]">Automação Inteligente</span>
                    </div>
                    Ao gerar o fechamento, o sistema irá recalcular todos os títulos em aberto (AR) do mês selecionado e consolidar o faturamento total. Este processo é reversível ao gerar novamente para o mesmo mês.
                  </div>
                  
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Mês de Referência *</label>
                    <div className="relative">
                      ...
                      />
                      <Calendar className="absolute right-6 top-1/2 -translate-y-1/2 text-muted-foreground/30 opacity-50" size={20} />
                    </div>
                  </div>
               </div>
               ...
               <div className="flex gap-4 pt-6 border-t border-border/50 relative">
                  ...
                  >
                    {generate.isPending ? <Loader2 size={16} className="animate-spin" /> : <><TrendingUp size={16} strokeWidth={3} /> Processar Balanço</>}
                  </button>
               </div>
            </div>
          </ScaleIn>
