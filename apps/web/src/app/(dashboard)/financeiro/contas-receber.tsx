...
import { 
  ...
  TrendingUp, Calendar, User, Receipt, Hash,
  CheckCircle2, X, AlertCircle, Ban
} from 'lucide-react';
...
const STATUS_MAP: Record<ArStatus, { label: string; cls: string; icon: any }> = {
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-6 py-4 truncate"># Ref / OS</th>
                  <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-6 py-4">Devedor (Cliente)</th>
                  <th className="text-right text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-6 py-4">Valor Bruto</th>
                  <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-6 py-4">Vencimento</th>
                  <th className="text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-6 py-4">Status Atual</th>
                  ...
                  <tr>
                    <td colSpan={6} className="py-24 text-center">
                       <div className="flex flex-col items-center gap-4">
                          ...
                          <Muted className="animate-pulse font-black uppercase tracking-[0.2em]">Consultando livro registro...</Muted>
                       </div>
                    </td>
                  </tr>
                ) : rows.length === 0 ? (
                  ...
                  <tr key={ar.id} className="group hover:bg-primary/[0.02] transition-colors">
                    <td className="px-6 py-5">
                      <div className="flex flex-col gap-1">
                        <span className="text-[10px] font-black tracking-widest text-muted-foreground/60 uppercase">Título F89-{ar.id}</span>
                        <div className="flex items-center gap-2">
                           <Hash size={14} className="text-primary/40" />
                           <span className="text-xs font-black text-foreground">OS#{ar.jobId}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                       <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-black text-foreground tracking-tight">{clientName ?? 'Não Identificado'}</span>
                       </div>
                    </td>
            ...
            <div className="premium-card p-10 flex flex-col gap-10 relative shadow-2xl border-destructive/20 overflow-hidden">
               ...
               <div className="flex justify-between items-start relative">
                 <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      <Muted className="text-[10px] font-black uppercase tracking-[0.2em]">Estorno ou exclusão de título financeiro</Muted>
                    </div>
                 </div>
                 <button 
               ...
               <div className="flex flex-col gap-4 relative">
                  <div className="p-4 bg-muted/50 rounded-2xl border border-border text-xs text-muted-foreground leading-relaxed italic">
                    "Esta conta a receber será marcada como cancelada. O valor não será contabilizado nos fluxos de caixa e o título continuará no histórico para auditoria."
                  </div>
                  
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Motivo do Estorno *</label>
                    <textarea
                      value={cancelReason}
               ...
               <div className="flex gap-4 pt-6 border-t border-border/50 relative">
                  ...
                  >
                    {cancelAr.isPending ? <Loader2 size={16} className="animate-spin" /> : 'Confirmar Cancelamento'}
                  </button>
               </div>
            </div>
          </ScaleIn>
