      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 mb-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            {schedule.driverName && <p className="text-zinc-400 text-sm mt-1">Motorista: <span className="text-white">{schedule.driverName}</span></p>}
            {schedule.vehicle && <p className="text-zinc-400 text-sm">Veículo: <span className="text-white">{schedule.vehicle}</span></p>}
          </div>
          {allScheduled && (
            <button
        ...
        <div className="flex gap-4 mt-4 text-xs text-zinc-500">
          ...
          <span><Clock size={12} className="inline mr-1 text-zinc-400" />{items.filter(i => i.item.status === 'scheduled').length} pendentes</span>
        </div>
      </div>
            ...
            <div key={item.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  <div className="w-7 h-7 bg-zinc-800 rounded-full flex items-center justify-center text-xs text-zinc-400 font-bold shrink-0">{idx + 1}</div>
                  <div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 w-full max-w-sm">
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={() => { setFailedItemId(null); setFailReason(''); }} className="flex-1 px-4 py-2 border border-zinc-700 text-zinc-300 rounded-lg text-sm hover:bg-zinc-800 transition-colors">Cancelar</button>
              <button
                onClick={() => {
