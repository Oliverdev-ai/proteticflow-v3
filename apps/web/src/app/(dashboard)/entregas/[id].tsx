...
import {
  ArrowLeft, Truck, CheckCircle, XCircle, Clock, AlertCircle, MapPin, Phone,
} from 'lucide-react';

const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  scheduled:  { label: 'Agendado',    color: 'text-neutral-400' },
...
export default function DeliveryDetailPage() {
  ...
  if (isLoading) return <div className="p-6 text-neutral-400">Carregando roteiro...</div>;
  ...
  const dateStr = new Date(schedule.date).toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' });
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-5 mb-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            {schedule.driverName && <p className="text-neutral-400 text-sm mt-1">Motorista: <span className="text-white">{schedule.driverName}</span></p>}
            {schedule.vehicle && <p className="text-neutral-400 text-sm">Veículo: <span className="text-white">{schedule.vehicle}</span></p>}
          </div>
          {allScheduled && (
            <button
        ...
        <div className="flex gap-4 mt-4 text-xs text-neutral-500">
          <span><CheckCircle size={12} className="inline mr-1 text-green-400" />{items.filter(i => i.item.status === 'delivered').length} entregues</span>
          <span><XCircle size={12} className="inline mr-1 text-red-400" />{items.filter(i => i.item.status === 'failed').length} falhas</span>
          <span><Clock size={12} className="inline mr-1 text-neutral-400" />{items.filter(i => i.item.status === 'scheduled').length} pendentes</span>
        </div>
      </div>
            ...
            <div key={item.id} className="bg-neutral-900 border border-neutral-800 rounded-xl p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  <div className="w-7 h-7 bg-neutral-800 rounded-full flex items-center justify-center text-xs text-neutral-400 font-bold shrink-0">{idx + 1}</div>
                  <div>
                    <p className="text-white font-medium">{clientName ?? '-'}</p>
                    {clientAddress && (
                      <p className="text-neutral-400 text-xs flex items-center gap-1 mt-0.5">
                        <MapPin size={11} /> {clientAddress}{clientNeighborhood ? `, ${clientNeighborhood}` : ''}
                      </p>
                    )}
                    {clientPhone && (
                    ...
                    <p className="text-neutral-500 text-xs mt-1">OS: {jobCode ?? item.jobId}</p>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-2">
                  ...
                  {(item.status === 'scheduled' || item.status === 'in_transit') && (
                    <div className="flex gap-2">
                      <button
                        onClick={() => updateStatus.mutate({ itemId: item.id, status: 'delivered' })}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 w-full max-w-sm">
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2 text-white text-sm placeholder-neutral-600 resize-none"
            />
            <div className="flex gap-3 mt-4">
              <button onClick={() => { setFailedItemId(null); setFailReason(''); }} className="flex-1 px-4 py-2 border border-neutral-700 text-neutral-300 rounded-lg text-sm hover:bg-neutral-800 transition-colors">Cancelar</button>
              <button
                onClick={() => {
                  if (!failReason.trim()) return;
                  updateStatus.mutate({ itemId: failedItemId, status: 'failed', failedReason: failReason });
                  setFailedItemId(null);
                  setFailReason('');
                }}
                className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium transition-colors"
              >Confirmar Falha</button>
            </div>
          </div>
        </div>
      )}
