      ...
      <div className="flex justify-between items-center bg-card/50 p-6 rounded-[32px] border border-border/50 backdrop-blur-sm shadow-sm">
        ...
        >
          {isAdding ? (
            ...
            <>
              <Plus size={14} strokeWidth={3} /> Adicionar Bloco
            </>
          )}
        </button>
      </div>
              ...
              {blocks.map((b) => (
                <tr key={b.id} className="group hover:bg-primary/[0.02] transition-colors">
                  ...
                  <td className="px-6 py-5 text-right">
                    <button
                      onClick={() =>
                        confirm('Remover este bloco de numeração?') &&
...
export default function ClientEditPage() {
  ...
  const { data: extract } = trpc.clientes.getExtract.useQuery(
    { id: clientId },
    { enabled: tab === 'extrato' },
  );
  ...
  } = useForm<ClientEditFormInput, unknown, ClientEditFormData>({
  ...
  const onSubmit: SubmitHandler<ClientEditFormData> = (data) =>
        ...
        <ScaleIn className="flex flex-col gap-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div
                key={card.label}
                className="premium-card p-8 flex flex-col gap-6 group hover:border-primary/30 transition-all"
              >
                <div
                  className={cn(
