...
export default function PricingTablesPage() {
  ...
  const { data, isLoading, error, refetch } = trpc.pricing.listTables.useQuery({
    ...
    <div className="flex flex-col gap-6 p-6 h-full overflow-auto">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-primary hover:bg-primary text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors"
        >
          ...
          <div className="bg-muted border border-border rounded-lg p-6 w-full max-w-sm space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                onClick={() => {
                  createMutation.mutate({ name: newName.trim(), isDefault: false });
                  setShowCreate(false);
                  setNewName('');
                }}
      ...
      {data?.data.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 gap-3">
          ...
          <p className="text-muted-foreground text-sm">Nenhuma tabela de preços.</p>
          <button
            onClick={() => setShowCreate(true)}
            className="text-xs text-primary hover:text-primary transition-colors"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {data?.data.map((table) => (
            <button
              ...
              className="bg-muted border border-border hover:border-primary/50 rounded-lg p-5 text-left transition-colors group"
            >
              ...
              {table.description && (
                <p className="text-muted-foreground text-xs line-clamp-2">{table.description}</p>
              )}
              <p className="text-xs text-muted-foreground mt-3">Ver itens →</p>
            </button>
          ))}
        </div>
      )}
