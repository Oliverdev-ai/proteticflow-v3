...
import { ArrowLeft, Plus, Search, Loader2, AlertCircle, Upload, Download, Percent, Trash2 } from 'lucide-react';
...
export default function PricingTableDetailPage() {
  ...
  const bulkMutation = trpc.pricing.bulkAdjust.useMutation({
    onSuccess: (r) => { setShowBulk(false); setBulkPercent(''); refetch(); alert(`${r.adjusted} itens reajustados!`); },
  });

  const createItemMutation = trpc.pricing.createItem.useMutation({
    onSuccess: () => { setShowAddItem(false); setNewItem({ name: '', category: '', priceCents: '', estimatedDays: '5', code: '' }); refetch(); },
  });
  ...
  const importMutation = trpc.pricing.importCsv.useMutation({
    onSuccess: (r) => { setImportResult(r); refetch(); },
  });
  ...
  const handleExport = async () => {
    ...
    if (res.data) {
      ...
      a.href = url; a.download = `tabela-${tableId}.csv`; a.click();
