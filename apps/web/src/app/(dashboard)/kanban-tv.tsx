...
const STATUS_THEMES: Record<JobStatus, { border: string; bg: string; text: string; iconBg: string }> = {
  pending: { border: 'border-muted', bg: 'bg-muted/10', text: 'text-muted-foreground', iconBg: 'bg-muted/20' },
    ...
    return (
      <div className="h-screen w-screen bg-background flex flex-col items-center justify-center gap-6">
        <div className="relative">
          ...
          <Monitor className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-primary" size={32} />
        </div>
        <p className="text-xl font-black uppercase tracking-[0.3em] text-foreground animate-pulse">Iniciando Monitor de Produção</p>
      </div>
    );
  }
    ...
    return (
      <div className="h-screen w-screen bg-background flex flex-col items-center justify-center gap-4">
        <div className="p-6 bg-destructive/10 rounded-3xl border border-destructive/20 text-destructive flex flex-col items-center gap-4">
          <Layers3 size={48} />
          <p className="text-2xl font-black uppercase tracking-widest">{boardQuery.error.message}</p>
        </div>
      </div>
    );
  }
    ...
    <PageTransition className="h-screen w-screen bg-[#050505] text-foreground overflow-hidden flex flex-col p-8 cursor-none">
      ...
      <header className="flex items-center justify-between pb-8 border-b border-border/50">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-sm font-bold uppercase tracking-[0.4em] text-muted-foreground opacity-60">Monitor de Chão de Fábrica</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-12">
          <div className="flex flex-col items-end gap-1">
            <div className="flex items-center gap-3 text-muted-foreground">
              <RotateCcw size={20} className={cn("transition-all", boardQuery.isFetching ? 'animate-spin text-primary' : 'opacity-40')} />
      ...
      <main className="flex-1 min-h-0 pt-8 flex gap-8">
        {columns.map((status) => {
          ...
          return (
            <div key={status} className="flex-1 flex flex-col bg-muted/5 rounded-[40px] border border-border/30 overflow-hidden backdrop-blur-sm shadow-2xl">
              <div className={cn("p-6 border-b border-border/50 flex items-center justify-between", theme.bg)}>
                <h2 className={cn("text-2xl font-black uppercase tracking-[0.2em]", theme.text)}>
                  {JOB_STATUS_LABELS[status]}
                </h2>
                <div className={cn("text-2xl font-black px-4 py-1 rounded-2xl border", theme.border, theme.text, "bg-background/50 shadow-sm")}>
                  {colJobs.length}
                </div>
              </div>
              ...
              <div className="flex-1 overflow-hidden p-6 flex flex-col gap-6 custom-scrollbar">
                {colJobs.slice(0, 6).map((job, idx) => (
                  <div 
                    ...
                    className={cn(
                      ...
                      theme.border,
                      theme.bg,
                      idx === 0 ? "scale-100 ring-4 ring-primary/20 brightness-110 shadow-primary/10" : "scale-[0.98] opacity-90"
                    )}
                  >
                    <div className="flex items-start justify-between">
                      <span className="text-3xl font-black tracking-tighter text-white">#{job.code}</span>
                      <div className={cn("px-4 py-1 rounded-xl text-sm font-black uppercase tracking-widest", theme.iconBg, theme.text)}>
                        {job.clientName ? `${job.clientName.toUpperCase().split(' ')[0]}` : 'S/ PARCEIRO'}
                      </div>
                    </div>
                    
                    <div className="space-y-1">
                       <p className="text-2xl font-bold text-foreground line-clamp-1">{job.patientName || 'PACIENTE GERAL'}</p>
                       {job.firstItemName && (
                    ...
                    <div className="mt-4 pt-4 border-t border-white/5 flex justify-between items-center">
                      ...
                      {job.deadline && (
                        <div className={cn(
                          ...
                          new Date(job.deadline) < new Date() ? "bg-destructive/20 text-destructive border-destructive/20" : "bg-primary/20 text-primary border-primary/20"
                        )}>
                          ENTREGA: {new Date(job.deadline).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {colJobs.length === 0 && (
                   <div className="flex-1 flex flex-col items-center justify-center opacity-20 gap-4">
                     ...
                     <p className="text-sm font-black uppercase tracking-[0.3em]">Nenhum item nesta fila</p>
                   </div>
                )}
              </div>
            </div>
      ...
      <footer className="pt-8 flex items-center justify-between text-muted-foreground">
        ...
        <div className="text-xs font-black uppercase tracking-[0.2em] opacity-40">
          Pressione <span className="text-primary px-2 py-1 bg-primary/10 border border-primary/20 rounded-md mx-1">ESC</span> para fechar monitor
        </div>
      </footer>
    </PageTransition>
  );
