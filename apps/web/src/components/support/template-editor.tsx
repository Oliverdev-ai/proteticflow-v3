  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-4 space-y-3">
        ...
        <input value={intent} onChange={(event) => setIntent(event.target.value)} placeholder="Intent (ex: status_query)" className="w-full rounded-lg bg-neutral-950 border border-neutral-800 px-3 py-2 text-sm text-neutral-100" />
        <input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Título interno" className="w-full rounded-lg bg-neutral-950 border border-neutral-800 px-3 py-2 text-sm text-neutral-100" />
        <textarea value={body} onChange={(event) => setBody(event.target.value)} placeholder="Resposta automática" className="w-full min-h-[90px] rounded-lg bg-neutral-950 border border-neutral-800 px-3 py-2 text-sm text-neutral-100" />
      ...
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-4 space-y-3">
        ...
        {templates.length === 0 ? <p className="text-xs text-neutral-500">Nenhum template cadastrado.</p> : null}
        {templates.map((template) => (
          <div key={template.id} className="rounded-xl border border-neutral-800 bg-neutral-950 p-3">
            <p className="text-sm text-white">{template.title}</p>
            <p className="text-xs text-neutral-400 mt-1">intent: {template.intent} • {template.isActive ? 'ativo' : 'inativo'}</p>
            <p className="text-sm text-neutral-300 mt-2 whitespace-pre-wrap">{template.body}</p>
            <div className="mt-3 flex gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded text-xs bg-neutral-700 text-white disabled:opacity-50"
              >
