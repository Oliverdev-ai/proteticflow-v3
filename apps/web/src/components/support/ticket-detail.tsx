  ...
  if (!ticket) {
    return (
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-6">
        <p className="text-sm text-neutral-500">Selecione um ticket para ver detalhes.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-4 space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs px-2 py-1 rounded bg-neutral-800 text-neutral-300">#{ticket.id}</span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button type="button" disabled={busy} onClick={() => void onUpdate({ status: 'closed' })} className="px-3 py-1.5 rounded text-xs bg-neutral-700 text-white disabled:opacity-50">Fechar</button>
      </div>

      <div className="space-y-2 max-h-[34vh] overflow-y-auto border border-neutral-800 rounded-xl p-3 bg-neutral-950">
        {messages.length === 0 ? <p className="text-xs text-neutral-500">Sem mensagens.</p> : null}
        {messages.map((message) => (
          <div key={message.id} className="border border-neutral-800 rounded-lg px-3 py-2">
      ...
      <div className="space-y-2">
        <textarea
          ...
          className="w-full min-h-[90px] rounded-xl bg-neutral-950 border border-neutral-800 text-neutral-100 text-sm px-3 py-2"
        />
