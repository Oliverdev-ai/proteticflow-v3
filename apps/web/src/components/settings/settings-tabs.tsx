...
const SETTINGS_TABS: TabItem[] = [
  ...
  { id: 'laboratorio', label: 'Laboratório', icon: Building2, component: <LabTab />, roles: ['superadmin', 'manager'] },
  { id: 'equipe', label: 'Time & Elenco', icon: Users, component: <EmployeesTab />, roles: ['superadmin', 'manager'] },
...
export function SettingsTabs() {
  ...
  return (
    <Tabs.Root value={activeTab} onValueChange={setActiveTab} className="flex flex-col lg:flex-row gap-8 min-h-[700px] w-full">
      {/* Sidebar Navigation */}
      <Tabs.List 
        className={cn(
      ...
      >
        <div className="bg-card/30 backdrop-blur-md border border-border/50 rounded-[32px] p-3 flex flex-col gap-1 shadow-2xl shadow-black/10 overflow-hidden sticky top-24">
           ...
           >
              {collapsed ? <ChevronRight size={18} /> : <div className="flex items-center gap-2 text-[8px] font-black uppercase tracking-widest"><ChevronLeft size={14} /> Recolher Painel</div>}
           </button>
           ...
           <div className="flex flex-col gap-1.5">
             {SETTINGS_TABS.map((tab) => (
               ...
               >
                 ...
                 {!collapsed && (
                   <span className="truncate animate-in fade-in slide-in-from-left-2 duration-500">{tab.label}</span>
                 )}
            ...
            <PageTransition key={tab.id}>
              <div className="flex flex-col gap-2 mb-10 ml-4 lg:ml-0">
                 ...
                 <div className="flex items-center gap-4">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/40 italic">Módulo de Configuração de Malha</span>
                 </div>
              </div>
              {tab.component}
            </PageTransition>
