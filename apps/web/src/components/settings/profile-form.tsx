...
export function ProfileForm() {
  ...
  // Keep existing reset flow (email reset)
  const resetCredentialMutation = trpc.auth.resetPasswordFromProfile.useMutation();

  // Add authenticated password-change flow (separate from reset)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-card/30 backdrop-blur-sm border border-border/50 rounded-[32px] p-8 flex flex-col gap-8 relative overflow-hidden group/profile shadow-xl shadow-black/5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
            ...
            <div className="flex flex-col gap-1">
              <label className={labelClass}>
                <Phone size={12} className="text-primary/40" /> Terminal Telefonico / WhatsApp
              </label>
            ...
            <div className="flex flex-col gap-1">
              <label className={labelClass}>
                <MapPin size={12} className="text-primary/40" /> Geo-Localizacao Referencial
              </label>
              <input className={inputClass} placeholder="Cidade/Estado" disabled value="Sao Paulo / SP" />
            </div>
          </div>
              ...
              {showChangePassword ? (
                <div className="rounded-2xl border border-border/60 bg-background/50 p-4 space-y-4">
                  <div className="space-y-3">
                    <div className="flex flex-col gap-1">
                      <label className={labelClass}>
                        <Lock size={12} className="text-primary/40" /> Senha Atual
                      </label>
                    ...
                    <div className="flex flex-col gap-1">
                      <label className={labelClass}>
                        <Key size={12} className="text-primary/40" /> Nova Senha
                      </label>
                      <div className="relative">
                        <input
                          type={showNewPassword ? 'text' : 'password'}
                          className={passwordInputClass}
                          placeholder="Min. 8 caracteres, 1 maiuscula e 1 numero"
                          value={newPassword}
                    ...
                    <div className="flex flex-col gap-1">
                      <label className={labelClass}>
                        <ShieldCheck size={12} className="text-primary/40" /> Confirmar Nova Senha
                      </label>
                      <div className="relative">
                        <input
                        ...
                        <button
                          ...
                          className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground transition-colors hover:text-primary"
                          aria-label={showConfirmPassword ? 'Ocultar confirmacao de senha' : 'Exibir confirmacao de senha'}
                        >
