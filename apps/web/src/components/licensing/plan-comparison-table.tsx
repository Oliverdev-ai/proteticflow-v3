          ...
          <tbody>
            {PLAN_ORDER.map((plan) => {
              ...
              return (
                <tr
                  key={plan}
                  className={isCurrent ? 'bg-emerald-950/30 border-y border-emerald-700/30' : 'bg-neutral-950'}
                >
                  ...
                  <td className="px-4 py-3 text-neutral-300">{formatLimit(limits.clients)}</td>
                  <td className="px-4 py-3 text-neutral-300">{formatLimit(limits.jobsPerMonth)}</td>
                  <td className="px-4 py-3 text-neutral-300">{formatLimit(limits.users)}</td>
                  <td className="px-4 py-3 text-neutral-300">{formatLimit(limits.priceTables)}</td>
                  <td className="px-4 py-3 text-neutral-300">{limits.features.portal ? 'Sim' : 'Nao'}</td>
                  <td className="px-4 py-3 text-neutral-300">{formatAiLabel(limits.features.ai)}</td>
                  <td className="px-4 py-3 text-neutral-300">{limits.features.api ? 'Sim' : 'Nao'}</td>
                  <td className="px-4 py-3 text-right">
                    {isCurrent ? (
                      ...
                      <button
                        ...
                        className="rounded-md bg-violet-600 hover:bg-violet-500 disabled:opacity-60 px-3 py-1.5 text-xs font-semibold text-white"
                      >
                        {loadingPlan === plan ? 'Abrindo...' : 'Assinar'}
                      </button>
                    ) : (
                      <span className="text-xs text-neutral-500">Nao disponivel</span>
                    )}
                  </td>
                </tr>
              );
