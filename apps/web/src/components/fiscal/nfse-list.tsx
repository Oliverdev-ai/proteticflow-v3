...
const STATUS_CLASSNAMES: Record<Nfse['status'], string> = {
  draft: 'bg-neutral-500/15 text-neutral-300 border border-neutral-500/30',
    ...
    <div className="rounded-2xl border border-neutral-800 bg-neutral-900 overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-neutral-800">
          {notas.map((nota) => (
            <tr key={nota.id}>
              <td className="px-4 py-3 text-neutral-200">#{nota.id}</td>
              <td className="px-4 py-3 text-neutral-200">{nota.tomadorName}</td>
              <td className="px-4 py-3 text-neutral-200">{formatCurrency(nota.grossValueCents)}</td>
              <td className="px-4 py-3">
                <span className={`inline-flex px-2 py-1 rounded-full text-xs ${STATUS_CLASSNAMES[nota.status]}`}>
                  {STATUS_LABELS[nota.status]}
                </span>
              </td>
              <td className="px-4 py-3 text-neutral-300">{nota.issuedAt ? formatDate(nota.issuedAt) : '-'}</td>
              <td className="px-4 py-3">
